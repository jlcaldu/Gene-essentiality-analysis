...
## Interactome
load("UnW.Interactome.graph.RData")

## Essentiality annotations
load(file="Essentiality.Annotations.RData")

## Functional properties
load("Annotated.Ensembl.gene.annotation.GRCh38.GRanges.RData")
ls()
...
### OGEE #####
OGEE_data <- list(devgenes = read.table("OGEE_devgenes.txt", sep='\t', header = T),
                   early_expr = read.table("OGEE_earliest_expression_during_development.txt", sep='\t', header = T),
                   duplications = read.table("OGEE_dupgenes.txt", sep='\t', header = T) )
...
#### Conservation index 
load("Conservation.index.GRanges.RData")
load("human.to.all.ortho.binary.matrix.RData")

### Diseases GWAS
GWAS <- readRDS('GWAS.mapped.genes.processed.rds')
...
protein.class <- read.csv("protein_classes.tsv",sep="\t", header = T, stringsAsFactors = F, fill=T)
...
GTEx.allExps <- getAtlasData("E-MTAB-5214")
...
```{r}
tmp <- read.csv('vivo_vitro_genes.csv', header=T)
...
### Supp. Fig 2. Evolutionary patterns
```{r a) Conservation index distribution}
tmp <- melt(Global.list$Figure7$Conservation.Score)
tmp$L2[is.na(tmp$L2)] <- "All"
