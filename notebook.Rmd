...
OGEE_data <- list(devgenes = read.table("OGEE_devgenes.txt", sep='\t', header = T),
                   early_expr = read.table("OGEE_earliest_expression.txt", sep='\t', header = T),
...
load("orthologs_matrix.RData")

### Diseases GWAS
GWAS <- readRDS('GWAS.mapped.genes.rds')
...
```{r}
tmp <- read.csv('neuronal_genes_annotation.csv', header=T)
